...
func TestProcessBlock_Transfer_DebitAndCredit(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := ProcessBlock(ctx, tx, 1000, fixedTime(), []rpc.Event{ev}, nil); err != nil {
...
func TestProcessBlock_Transfer_EmptyAmountIsNoOp(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := ProcessBlock(ctx, tx, 1001, fixedTime(), []rpc.Event{ev}, nil); err != nil {
			t.Fatalf("process: %v", err)
		}
...
func TestProcessBlock_Transfer_MissingAmountIsNoOp(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := ProcessBlock(ctx, tx, 1002, fixedTime(), []rpc.Event{ev}, nil); err != nil {
			t.Fatalf("process: %v", err)
		}
...
func TestProcessBlock_Coinbase(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := ProcessBlock(ctx, tx, 1010, fixedTime(), []rpc.Event{ev}, nil); err != nil {
			t.Fatalf("process: %v", err)
		}
...
func TestProcessBlock_Burn(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := ProcessBlock(ctx, tx, 1020, fixedTime(), []rpc.Event{ev}, nil); err != nil {
			t.Fatalf("process: %v", err)
		}
...
func TestProcessBlock_Delegate_ThreeRowPattern(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := ProcessBlock(ctx, tx, 1030, fixedTime(), nil, []rpc.TxResult{{Events: []rpc.Event{ev}}}); err != nil {
			t.Fatalf("process: %v", err)
		}
...
func TestProcessBlock_Redelegate_CrossEventAndDefusion(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := ProcessBlock(ctx, tx, 1040, fixedTime(), nil, []rpc.TxResult{{Events: []rpc.Event{redel, wr}}}); err != nil {
			t.Fatalf("process: %v", err)
		}
...
func TestProcessBlock_CompleteRedelegation(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := ProcessBlock(ctx, tx, 1050, fixedTime(), []rpc.Event{ev}, nil); err != nil {
			t.Fatalf("process: %v", err)
		}
...
func TestProcessBlock_Unbond_FourRowsPlusDefusion(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := ProcessBlock(ctx, tx, 1060, fixedTime(), []rpc.Event{ev}, nil); err != nil {
			t.Fatalf("process: %v", err)
		}
...
func TestProcessBlock_CancelUnbond(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := ProcessBlock(ctx, tx, 1070, fixedTime(), []rpc.Event{ev}, nil); err != nil {
			t.Fatalf("process: %v", err)
		}
...
func TestProcessBlock_CompleteUnbonding_ReturnsBaseDenom(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := ProcessBlock(ctx, tx, 1080, fixedTime(), []rpc.Event{ev}, nil); err != nil {
			t.Fatalf("process: %v", err)
		}
...
func TestProcessBlock_CreateValidator_CrossEventSpender(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := ProcessBlock(ctx, tx, 1090, fixedTime(), nil, []rpc.TxResult{{Events: []rpc.Event{cv, spend}}}); err != nil {
			t.Fatalf("process: %v", err)
		}
...
func TestProcessBlock_NonBankEventsAreSkipped(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := ProcessBlock(ctx, tx, 1100, fixedTime(), evs, nil); err != nil {
			t.Fatalf("process: %v", err)
		}
