...
func TestHandler_Grid_OreUpsert_WritesStat(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := (gridHandler{}).Handle(ctx, tx, bctx(), raw); err != nil {
...
func TestHandler_Grid_DeleteWritesZeroSentinel(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := (gridHandler{}).Handle(ctx, tx, bctx(), ins); err != nil {
			t.Fatalf("seed: %v", err)
		}
		...
		if err := (gridHandler{}).Handle(ctx, tx, bctx(), del); err != nil {
...
func TestHandler_StructAttribute_HealthUpsert_WritesStat(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := (structAttributeHandler{}).Handle(ctx, tx, bctx(), raw); err != nil {
			t.Fatalf("upsert: %v", err)
		}
		var atype, otype, oid string
		var subIdx int
...
func TestHandler_StructAttribute_ZeroValueDeletes(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := (structAttributeHandler{}).Handle(ctx, tx, bctx(), seed); err != nil {
			t.Fatalf("seed: %v", err)
		}
		// value="0" should DELETE per the 20260203 migration.
		zero := mustJSON(t, map[string]any{"attributeId": "0-5-9999", "value": "0"})
		if err := (structAttributeHandler{}).Handle(ctx, tx, bctx(), zero); err != nil {
...
func TestHandler_StructAttribute_StatusBit32_StampsDestroy(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := (structHandler{}).Handle(ctx, tx, bctx(), structRaw); err != nil {
		...
		if err := (structAttributeHandler{}).Handle(ctx, tx, bctx(), statusRaw); err != nil {
			t.Fatalf("status upsert: %v", err)
		}
...
func TestHandler_PlanetAttribute_UpsertAndDelete(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := (planetAttributeHandler{}).Handle(ctx, tx, bctx(), ins); err != nil {
		...
		if err := (planetAttributeHandler{}).Handle(ctx, tx, bctx(), upd); err != nil {
		...
		if err := (planetAttributeHandler{}).Handle(ctx, tx, bctx(), zero); err != nil {
			t.Fatalf("zero delete: %v", err)
		}
		var n int
...
func TestHandler_PlanetAttribute_EmptyValueDeletes(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		if err := (planetAttributeHandler{}).Handle(ctx, tx, bctx(), del); err != nil {
...
func TestHandler_PlanetAttribute_AllLabels(t *testing.T) {
	...
	inTx(t, conn, func(tx pgx.Tx) {
		...
		for attrType, want := range planetAttrLabels {
			...
			if err := (planetAttributeHandler{}).Handle(ctx, tx, bctx(), id); err != nil {
				t.Fatalf("attrType=%d: %v", attrType, err)
			}
			var got string
			_ = tx.QueryRow(ctx,
