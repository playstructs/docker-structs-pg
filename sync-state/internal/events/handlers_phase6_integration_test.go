...
// TestPhase6_Raid_RaidStatusCarriesBlockStartRaid verifies that the
// raid_status detail folds in the planet's current block_start_raid (the
// sibling blockStartRaid attribute, id "10-<planet>"): the active raid block
// when one exists, and 0 when no blockStartRaid attribute is set.
func TestPhase6_Raid_RaidStatusCarriesBlockStartRaid(t *testing.T) {
	conn := connect(t)
	inTx(t, conn, func(tx pgx.Tx) {
		ctx := context.Background()
		suppressTriggers(t, tx)
		...
		// Planet 558: blockStartRaid = 700003 is set before the raid_status
		// event, so the detail must carry it.
		if err := (planetAttributeHandler{}).Handle(ctx, tx, bc,
			mustJSON(t, map[string]any{"attributeId": "10-2-558", "value": "700003"})); err != nil {
			t.Fatalf("set blockStartRaid: %v", err)
		}
		if err := (raidHandler{}).Handle(ctx, tx, bc,
			...
			t.Fatalf("raid with-block: %v", err)
		}
		flushBuf(t, ctx, tx, bc)
		...
		if err := tx.QueryRow(ctx,
			`SELECT (detail->>'block_start_raid')::bigint
			  ...
			  WHERE planet_id='2-558' AND category='raid_status'
			  ORDER BY seq DESC LIMIT 1`).Scan(&withRaid); err != nil {
			t.Fatalf("query with-block: %v", err)
		}
		if withRaid != 700003 {
			t.Errorf("block_start_raid with active raid = %d want 700003", withRaid)
		}

		// Planet 559: no blockStartRaid row exists, so block_start_raid must
		...
		if err := tx.QueryRow(ctx,
			`SELECT (detail->>'block_start_raid')::bigint
			   FROM structs.planet_activity
			  WHERE planet_id='2-559' AND category='raid_status'
			  ORDER BY seq DESC LIMIT 1`).Scan(&noRaid); err != nil {
			t.Fatalf("query no-block: %v", err)
		}
		if noRaid != 0 {
